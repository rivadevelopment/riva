   ...
   <!--******************************** -->   
    ...
    <!--************ flash ************* -->
        ...
        <!--    <a id="sMobile" class="btn collapse-btn theme-btn-hover-gray text-center align-items-center font-size-15 border bg-8 p-3" style="width:100%;">-->
        <!--       <span style="color:red;">Advance Search </span>-->
        <!--       <i id="arrow-fff" class="la la-angle-down ml-1" style="color:red">-->
        <!--       </i>-->
        <!--    </a>-->
        <!--</div>   -->
...
<div class="mobileShow">
     <div class="container align-items-center text-center">
         <div class="main_search contact-form-action align-items-center">
                   ...
                   <div class="col-md-12 align-items-center">
                     <div class="input-wrapper align-items-center">
                             <!--<button class="btn btn-outline-dark" data-bs-toggle="modal" data-bs-target="#searchArea">-->
                              <button class="btn btn-outline-dark p-2" id="BtnadvanceFilter" style="width:100%;">
                                 Advanced Search 
                             </button>
                      </div>
                   </div>
    ...
    <div class="col-md-3 d-none d-sm-block" id="fadein2">
    ...
    $SegType = $item->segments[0][0]->type;

    if (type == "oneway"){
         if ($item->segments[0][0]->type = 'manual') {
                $flight = [
                ...
                'departure_flight_no'=>$item->segments[0][0]->departure_flight_no,
                'oneway_origial_price'=>$item->segments[0][0]->Originalprice,
                'currency'=> $currency,
              ...
              $flight = [
                ...
                'currency'=> $currency,
                //'total'=> adults * $item->segments[0][0]->adult_price + childs * $item->segments[0][0]->child_price + infants * $item->segments[0][0]->infant_price,
                'total'=>  $item->segments[0][0]->adult_price + $item->segments[0][0]->child_price + $item->segments[0][0]->infant_price,
                'supplier'=> $item->segments[0][0]->type,
                'flight_type'=> type
                ];
         }
    ...
    if (type == "return"){
     if ($item->segments[0][0]->type = 'manual') {
        ...
        'return_origial_price'=>$item->segments[1][0]->Originalprice,
        'departure_flight_no'=>$item->segments[1][0]->departure_flight_no,
        'currency'=> $currency,
        'total'=>  (adults *$item->segments[0][0]->adult_price + adults *$item->segments[0][0]->child_price + adults *$item->segments[0][0]->infant_price),
        //'total'=> adults * $item->segments[0][0]->adult_price + childs * $item->segments[0][0]->child_price + infants * $item->segments[0][0]->infant_price
       ...
       //$price = $price_oneway + $price_return;
        ...
        if ($item->segments[0][0]->type = 'manual') {
         ...
         $price = $flight['oneway_adult_price']+$flight['oneway_child_price']+$flight['oneway_infant_price'];
        }
        //$price =  $flight['price'];
...
<!-- ////////////////////////////////////////////////////// -->
    ...
    <?php foreach ($item->segments as $index => $segment) { /*dd($segment); die;*/  // dd($segment); ?>
    ...
    <?php if(isset($segment[0]->session_id)){ 
                $session_search_id = $segment[0]->session_id; 
            } 
            else {
                $session_search_id = ""; 
            }  
            if(isset($segment[0]->fare_source_code)){
                $fare_source_code = $segment[0]->fare_source_code;
            }
            else{
                $fare_source_code = "";   
            }
  ...
  <!----------------===================== ===== ====================------------------------>
    ...
    <input type="hidden" name="session_id" value="<?php echo $session_search_id; ?>">
    <input type="hidden" name="fare_source_code" value="<?php echo $fare_source_code; ?>">
        ...
        <div class="theme-search-results-item-flight-section">
            <div class="row-no-gutter row-eq-height row">
                ...
                <div class="col-md-10 col-12">
                       <?php 
                            $date1 = $segment[0]->departure_date;
                            $time1 = $segment[0]->departure_time;
                            $date2 = end($segment)->arrival_date;
                            $time2 = end($segment)->arrival_time;
                            
                            // Combine date and time strings to create the DateTime objects
                    ...
                    <div class="theme-search-results-item-flight-section-item">
                        <div class="row">
                            ...
                            <div class="col-md-6 col-6 g-0">
                                <div class="theme-search-results-item-flight-section-path">
                                    <div class="theme-search-results-item-flight-section-path-fly-time">
                                         ...
                                         <p><strong><?=T::flights_tripduration?> <?=$timeFormatted?> <?=T::hours?></strong></p>
                                    </div>
                            ...
                            <div class="col-md-3 col-3">
                                <div class="theme-search-results-item-flight-section-meta">
                                    <p class="theme-search-results-item-flight-section-meta-time"><?= date ('h:i a',strtotime(end($segment)->arrival_time));  ?></p>
                                    ...
                                    <p class="theme-search-results-item-flight-section-meta-date"><?= date ('d M Y',strtotime(end($segment)->arrival_date));  ?></p>
                                </div>
                            </div>
                        </div> <!-- row -->
                        <!--<div class="row">-->
                        <!--      <a href="#" class=" link-primary waves-effect" style="text-decoration: underline;">-->
        ...
        <div class="row">
            <div class="col-md-3">
                <div class="theme-search-results-item-flight-details-info p-3">
                    ...
                    <p class="theme-search-results-item-flight-details-info-date"><?= date ('d M Y',strtotime($segment_collapse->departure_date)); ?></p>
                    <p class="theme-search-results-item-flight-details-info-cities"><?= $segment_collapse->departure_airport ?></p>
                    <p class="theme-search-results-item-flight-details-info-fly-time"><?= date ('h:i a',strtotime($segment_collapse->departure_time)); ?></p>
                </div>
            </div>
            <div class="col-md-9">
                <div class="theme-search-results-item-flight-details-schedule">
                    <ul class="theme-search-results-item-flight-details-schedule-list">
                        <li>
                            ...
                            <p class="theme-search-results-item-flight-details-schedule-date"><?=T::to?> <?= date ('d M Y',strtotime($segment_collapse->departure_date)); ?></p>
                            <div class="row">
                            <div class="col-5">
                            ...
                            <div class="theme-search-results-item-flight-details-schedule-time">
                                <span class="theme-search-results-item-flight-details-schedule-time-item">
                                    <?= date ('h:i a',strtotime($segment_collapse->departure_time)); ?>
                                    <!--<span>pm</span>-->
                                </span>
                                <span class="theme-search-results-item-flight-details-schedule-time-separator">-</span>
                                <span class="theme-search-results-item-flight-details-schedule-time-item">
                                    <?= date ('h:i a',strtotime($segment_collapse->arrival_time)); ?>
                                    <!--<span>pm</span>-->
                                </span>
                            </div>
                            <p class="theme-search-results-item-flight-details-schedule-fly-time">
                              ...
                              <?php
                                $hours = floor($segment_collapse->duration_time / 60);
                                $minutes = $segment_collapse->duration_time % 60;
                                $timeFormatted = sprintf("%02d:%02d", $hours, $minutes);
                              ?>
                              <?= $timeFormatted ?>
                              <?=T::hours?>
                            ...
                            <p><strong>Class</strong></p> <p><?=$segment_collapse->class_type?></p> 
    ...
    <script>
        ...
        var seen = {};
        $(".remove_duplication").find("li").each(function(index, html_obj) { 
        ...
        $rangeA.ionRangeSlider({
                                    ...
                                    if(isset($segment)) {  if ($SegType = 'manual') { ?>
                                        min: 100,
                                        max: 500000,
                                        from:100,
                                        to: 500000,
                                    ...
                                    <?php  } else { ?>
                                        min: <?php foreach ($flights_data as $index => $item) {$segment[$index] = $item->segments[0][0]->price;}  echo  $min_price = min($segment); ?>,
                                        max: <?php foreach ($flights_data as $index => $item) {$segment[$index] = $item->segments[0][0]->price;}  echo  $min_price = max($segment); ?>,
                                        from: <?php foreach ($flights_data as $index => $item) {$segment[$index] = $item->segments[0][0]->price;} echo  $min_price = min($segment); ?>,
                                        to: <?php foreach ($flights_data as $index => $item) {$segment[$index] = $item->segments[0][0]->price;}   echo  $min_price = max($segment); ?>,
                                        onChange: handleRangeInputChange 
                                 <?php   } }?>
                              });
