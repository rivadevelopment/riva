...
foreach ($routes[0] as $index => $segment ) { ?>
    ...
    <div>
        <div class="ItineraryTrip-title"><span class="ItineraryTrip-depart"><span class="">
            <?=$segment->departure_code?> <i class="la la-arrow-right"></i> <?=$segment->arrival_code?></span></span><span class="ItineraryTrip-time _actionButtons" ><span><?=T::flights_duration?>: </span>
            <span>
                <time class="">
                    <?php 
                        $hours = floor($segment->duration_time / 60);
                        $minutes = $segment->duration_time % 60;
                        $timeFormatted = sprintf("%02d:%02d", $hours, $minutes);
                    ?>
                    <?=$timeFormatted?>
                    <!--?=$segment->duration_time;?--> <?=T::hours?>
                </time>
                ...
                <div class="ItineraryDate">
                    <div class="Stack__StyledStack-sc-1t576ow-0 fuCtbb">
                        <div>
                            ...
                            <span class="ItineraryDate-date spTypo-medium"><time class="">
                                <?php $d = date('d D M Y', strtotime($segment->departure_date)); echo $d ?> - <small><?=$segment->departure_time?></small></time></span>
                        </div>
