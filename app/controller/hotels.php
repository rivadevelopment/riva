...
$router->get($session_langcur . hotels . '(.*)', function () {
    ...
    $req1 = new Curl();
    $req1->post(api_url.'api/hotel/location_details?appKey='.api_key, $parms);   
    
    if ($req1->error) {   //echo 'Error: ' . $req->errorCode . ': ' . $req->errorMessage . "\n";
        dd('Error, please log the issue with the administrators'); die;
    } else { 
       $city_loc_data = $req1->response;  
     //  dd($req1->response); die;     
    }
    $parms['countryName'] = $city_loc_data[0]->country;
    dd($parms);die;
...
$router->post('hotels/book', function () {
    ...
    // dd('BELOW ahmad_test_data');
    // dd($_SESSION[$_POST['merchant_extra1']]);   
    // dd($_SESSION['ahmad_test_data']); 
    ...
    // dd('YES');
    // exit();
    // if(isset($_SESSION[$_POST['merchant_extra1']])) {
    //     dd($_SESSION[$_POST['merchant_extra1']]);
    // } else {
    //     dd($_SESSION);
    // }
    ...
    $user_id = user_id();
    // $user_id = isset($_SESSION['user_login']) ? isset($_SESSION['user_id']) : "0";
...
$router->post('hotels/booking/invoice/(.*)', function () {
    ...
    if($booking && $booking->booking_payment_gateway == 'payment-with-credit-card') {
        if($_POST['response_message'] == 'Success') {
            ...
            // exit();
            if ($req->response == true) {
               header('Location:'.root.'hotels/booking/invoice/'.$_POST['merchant_extra1']);
            } else {
    ...
    $invoice_url =  root . 'hotels/booking/invoice/'.$url[3].'/'.$url[4];
    header('Location: ' .$invoice_url);
});
