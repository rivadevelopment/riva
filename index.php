...
// // send email
// echo '-->'. mail("arslan.dev231@gmail.com","My subject",$msg);
...
session_start();
if(!isset($_SESSION['amazon_pay_data'])){
  $_SESSION['amazon_pay_data'] = [];
}
...
if (empty($_SESSION['app']->app)) { echo '<body style="display: flex; justify-content: center; align-items: center; font-weight: bold; font-family: monospace; font-size: 16px; background: #e1e5eb; color: #0329b1">No Response from API Server</body>';
  //  session_destroy();
    die; };
...
//var_dump($method, $path, $statusCode, $exception);
if ($statusCode == 500) {
  $body = views."file.php";
  //include layout;
}else {
...
$router->get('sd',function() { 
  // session_destroy();
   echo '<strong>Session Destroyed</strong><meta http-equiv="refresh" content="1; URL='.root.'"/>';
});
