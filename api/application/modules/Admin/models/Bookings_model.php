    ...
    function admin_get_all_bookings($booking_status,$booking_payment_status) {
        ...
        foreach ($hotels_bookings as $hotels) {
            array_push($hotel_arr,(object) [
                ...
                'transaction_id'=>$hotels->transaction_id,
                'transaction_status'=>$hotels->transaction_status,
                'module'=>'hotels',
        ...
        foreach ($flights_bookings as $flights) {
            array_push($flights_arr,(object) [
                ...
                'transaction_id'=>$flights->transaction_id,
                'transaction_status'=>$flights->transaction_status,
                'module'=>'flights',
        ...
        foreach ($tours_bookings as $tours) {
            array_push($tours_arr,(object) [
                ...
                'transaction_id'=>$tours->transaction_id,
                'transaction_status'=>$tours->transaction_status,
                'module'=>'tours',
                'ai_first_name'=>$tours->tour_name,
                'supplier_name'=>$tours->supplier_name,
                'booking_from'=> $tours->booking_from,
                'booking_status'=>$tours->booking_status,
                'booking_payment_status'=>$tours->booking_payment_status,
                'booking_date'=>$tours->booking_date,
                'booking_curr_code'=>$tours->booking_curr_code,
                'total_price'=>$tours->total_price
            ]);
        }
        return array_merge($hotel_arr, $tours_arr, $flights_arr);
    }
...
/*END booking status update*/
    ...
    public function do_tours_booking($payload = array())
    {
        ...
        $itemid  = $payload['tour_id'];
        $adults  = $payload['travellers'];
        $child   =  0;
        $infant  = 0;
        $apiDate = $payload['departure_time'];
        ...
        $adults_price = $payload['b2c_price_adult'] * $payload['adults'];
        $childs_price = $payload['b2c_price_child'] * $payload['childs'];
        $infants_price = $payload['b2c_price_infant'] * $payload['infants'];
        $sub_total = $adults_price + $childs_price + $infants_price;

         if ($payload['taxType'] == "percentage" ) {
            $tax = $payload['taxValue'];
            $total = $sub_total;
            $totaltax = ($tax / 100) * $total; 
            
         }

        if ($payload['taxType'] == "fixed" ) {
            $tax = $payload['taxValue']; 
            $total = $sub_total;
            $totaltax = $tax;
        ...
        if($payload['payment_gateway']=="installments"){
            $payment_status = 'paid';
        ...
        $total_price = $sub_total + $totaltax;
        // dd($payload);die;
        $user_id = isset($payload['user_id'])? $payload['user_id'] : $payload['userId'];

        $data = array(
            'booking_ref_no' => $refno,
            'booking_supplier' => $payload['booking_supplier'],
            'tour_id' => $payload['tour_id'],
            'tour_name' => $payload['name'],
            'deposit_type' => $payload['taxType'],
            'tax_type' => $payload['taxType'],
            'total_price' => $total_price,
            'booking_date' => time(),
            'booking_user_id' => $user_id,
            'booking_remaining' => $total_price,
            ...
            'booking_infants' => $payload['infants'],
            'booking_adults' => $payload['adults'],
            'booking_childs' =>$payload['childs'],
            'booking_deposit' => $total_price,
            'booking_tax' => $payload['taxValue'],
            'booking_curr_code' => $payload['currencycode'],
            'tour_img' => $payload['img'],
            'tour_loaction' => $payload['location'],
            ...
            'booking_key' => "",
            'tour_stars' => $payload['rating'],
            'supplier_name' => 'manual',
            'tour_phone' => $payload['phone'],
            'tour_email' => $payload['email'],
    ...
    public function hotels_booking($payload = array())
    {
       // dd($payload); die;
        ...
        $checkin    = date('Y-m-d', strtotime($payload['booking_checkin']));
        $checkout   = date('Y-m-d', strtotime($payload['booking_checkout']));
        if($payload['booking_payment_gateway']=="installments"){
            $payment_status = 'paid';
            $booking_status = 'confirmed';
        } else {
            $payment_status = 'unpaid';
    ...
    function cancelbookingTour($payload = array()){
        $this->db->where('booking_id',$payload['id']);
...
//dd($setting_id);
    ...
    {
        $refno = random_string('numeric', 4);
        if($payload['booking_payment_gateway']=="tabby" || $payload['booking_payment_gateway']=="debit::credit-card-payment" || $payload['booking_payment_gateway']=="debit::credit-card-installment"){
            $payment_status = 'paid';
            $booking_status = 'confirmed';
        } else {
            $payment_status = 'unpaid';
        ...
        $data = array(
            ...
            'booking_user_id' => $payload['booking_user_id'],
            //'booking_remaining' => $payload['total_price'],
            'booking_amount_paid' => $payload['total_price'],
            'booking_remaining' => '0',
            'booking_status' => $booking_status,
            'transaction_id' => $payload['transaction_id'],
            'transaction_status' => $payload['transaction_status'],
            ...
            'integration_book_status' => $payload['integration_book_status'],
        );
    ...
    public function do_flights_booking($payload = array())
    {
        ...
        // $currCode = $bookingData->currCode; // ferassssss
