...
//cars assignment
        ...
        function user_dashboard($id)
    {
        ...
        $this->db->where('accounts_id', $id);    
        $data_rr2 = $this->db->get()->result_array();
        //
        $this->db->select();
        $this->db->from('pt_accounts');
        $this->db->where('accounts_email', $data_rr2[0]['accounts_email']); 
        $data_rr = $this->db->get()->result_array();    
        // dd($data_rr);die;
        $data_rr_id = [];
        foreach($data_rr as $data_rray){
            $data_rr_id[] = $data_rray['accounts_id'];
        }
        //end add by ak
        ...
        $this->db->where_in('booking_user_id', $data_rr_id);
        ...
        $tour_totel_booking = $this->db->get()->num_rows();

        $this->db->select('booking_status');
        $this->db->where_in('booking_user_id', $data_rr_id);
        $this->db->where('booking_status', 'pending');
        $this->db->from('tours_bookings');
        $tour_pending_nvoices = $this->db->get()->num_rows();
        ...
        return $dashboard = array(
            ...
            'totel_booking'=> $flight_totel_booking+$hotel_totel_booking+$tour_totel_booking,
            'pending_nvoices'=> $hotel_pending_nvoices+$flight_pending_nvoices+$tour_pending_nvoices
        );
...
// List all bookings on account my bookings page
    function get_my_bookings($invoice_id) {
        ...
        foreach ($tours_bookings as $tours) {
            array_push($tour_arr,(object) [
                'booking_id'=>$tours->booking_id,
                'booking_ref_no'=>$tours->booking_ref_no,
                'module'=>'tours',
                'paymethod'=>$tours->booking_payment_gateway,
                'nights'=>$tours->booking_nights,
                'checkin'=>$tours->booking_checkin,
                'checkout'=>$tours->booking_checkout,
                'date'=>pt_show_date_php($tours->booking_date),
                'currCode'=>$tours->booking_curr_code,
                'currSymbol'=>'',
                'checkoutAmount'=>$tours->booking_deposit,
                'checkoutTotal'=>$tours->total_price,
                'status'=>$tours->booking_status,
                'bookingUser'=>$tours->booking_user_id,
                'userFullName'=>$tours->ai_first_name . " " . $tours->ai_last_name,
                'accountEmail'=>$tours->accounts_email,
                'userMobile'=>$tours->ai_mobile,
                'userCountry'=>$tours->ai_country,
                'userAddress'=>$tours->ai_address_1. " " . $tours->ai_address_2,
                "additionaNotes" => $tours->booking_additional_notes,
                "couponCode" => $tours->booking_coupon,
                "remainingAmount" => $tours->booking_remaining,
                "guestInfo" => json_decode($tours->booking_guest_info),
                'tax'=>$tours->booking_tax,
                'amountPaid'=>$tours->booking_amount_paid,
                'booking_payment_status'=>$tours->booking_payment_status,
            ]);
        }
        //
        $flights_arr = [];
        ...
        return array_merge($hotel_arr,$flights_arr,$tours_arr);
    }
...
// List all user bookings on account my bookings page
    function get_user_bookings($id) {
        ...
        // $this->db->where_in('pt_accounts.accounts_id','select accounts_id from pt_accounts where accounts_email = (select accounts_email from pt_accounts where accounts_id =398)');

            $this->db->where_in('pt_accounts.accounts_id',$data_rr_id);
        }
        ...
        foreach ($tours_bookings as $tours) {
            array_push($tour_arr,(object) [
                ...
                'booking_ref_no'=>$tours->booking_ref_no,
                'module'=>'tours',
                'ai_first_name'=>$tours->ai_first_name,
                'booking_status'=>$tours->booking_status,
                'booking_payment_status'=>$tours->booking_payment_status,
                'booking_date'=>pt_show_date_php($tours->booking_date),
                'booking_curr_code'=>$tours->booking_curr_code,
                'total_price'=>$tours->total_price
            ]);
        }
        // 
        $flights_arr = [];
        ...
        // return $tour_arr;
        return array_merge($hotel_arr,$flights_arr,$tour_arr);
    }
