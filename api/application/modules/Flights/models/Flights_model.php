...
if($triptype != "oneway")
    ...
    function get_country_phonecode($code)
    {
        $this->db->select("pt_flights_countries.*");
        $this->db->where('iso3',$code);
        return $this->db->get('pt_flights_countries')->row()->phonecode; 

    }
    
     function get_country_areacode($code)
    {
        $this->db->select("pt_flights_countries.*");
        $this->db->where('iso3',$code);
        return $this->db->get('pt_flights_countries')->row()->numcode;

    }
     function get_country_iso($nat)
    {
        $this->db->select("pt_flights_countries.*"); 
        $this->db->where('LOWER(name)',strtolower($nat));
        return $this->db->get('pt_flights_countries')->row()->iso;
