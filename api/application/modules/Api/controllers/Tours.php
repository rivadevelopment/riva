...
class Tours extends REST_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('Api/Apitours_model');
    ...
    function cancellationbooking_post(){
            $payload = $this->post();
            $this->load->model('Admin/Bookings_model');
            $data = $this->Bookings_model->cancelbookingTour($payload);
            $this->response($data, 200);
    }

        /*/////////////tours booking invoice//////////////*/
    function booking_get(){

        $invoice_id = $this->get('invoice_id');
        $booking_id = $this->get('booking_id');

        $booking = $this->Apitours_model->get_my_toursbooking($invoice_id,$booking_id);
        // dd($booking);die;
        if(!empty($booking)){
