...
//set tour id by id
  ...
  function tour_details($tourid = null, $date = null) {   // Feras chnage 
    if (empty($tourid)) {
      $tourid = $this->tourid;
    }
    else {
    ...
    $tour_validity_from = $details[0]->tour_validity_from;
    $tour_validity_to = $details[0]->tour_validity_to;
...
//	$isfeatured = $this->is_featured();
    ...
    $detailResults = (object) 
         array('id' => $details[0]->tour_id, 
               ...
               'taxValue' => $tax_value, 
               'tourDays' => $tourDays, 
               'tourValidityFrom' => $tour_validity_from, 
               'tourValidityTo' => $tour_validity_to,  
               'tourhours' => $tourhours, 
...
// get tour images
  ...
  function getFeaturedTours() {
    ...
    function getFeaturedToursmain($lang,$currency) { /// Featured Tours --- Feras
...
// Tour visiting Cities
  ...
  function featured_tours_list() {  /// Feras Featured 
    $settings = $this->settings();
    $limit = $settings[0]->front_homepage;
    $orderby = $settings[0]->front_homepage_order;
   
    $this->db->select('tour_id,tour_order,tour_title,tour_status,tour_location,tour_featured_forever,tour_status');
    $this->db->where('tour_is_featured', 'yes');
    $this->db->where('tour_featured_from <', time());
    $this->db->where('tour_featured_to >', time());
    $this->db->or_where('tour_featured_forever', 'forever');
...
//make a result object all data of tours array
  function getResultObject($tours) { //// feras - featured 
    $this->ci->load->library('currconverter');
    $result = array();
    $curr = $this->ci->currconverter;
    foreach ($tours as $t) {
      ...
      $result[] = (object) array(
          'id' => $this->tourid,
          'title' => $this->title, 
          'slug' => base_url() . 'tours/' . $this->slug, 'thumbnail' => $this->thumbnail, 'stars' => pt_create_stars($this->stars), 
...
//$this->db->or_like('pt_locations.country',$query);
  ...
  public function invoceurlupdate($id,$invoice_url){
    ...
    public function response_booking($id,$bookconf){
        ...
        $this->db->where('booking_id',$id);
        $getinovice_link = $this->db->get('tours_bookings')->result();
    ...
    public function get_booking($id){
      $this->db->where('booking_id',$id);
     return $this->db->get('tours_bookings')->result();
    }
}
